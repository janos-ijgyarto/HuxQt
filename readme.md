...
Todas las operaciones necesarias pueden realizarse atrvez de de la herramienta, minimizando el riesgo causado por una sintaxis incorrecta y reduciendo el costo de tiempo para la creación y edición de terminales.
...
_NOTA: Hux proporciona una representación cercana, pero no 1:1 de la representacion de la terminal de Aleph One. Se conserva todo el contenido del scripteado, pero puede haber pequeñas diferencias en el resultado mostrado (por ejemplo, ajustes de líneas)_
...
Una vez que abra Hux, primero debes [importar](#importing-scenarios) un escenario desde la capreta dvidida (split folder), or [abrir](#loading-scenarios) un archivo de escenario Hux.
...
Para importar un escenario desde una carpeta dividida (split folder), simpemente haga click en _File -> Import Scernacio Scrips_ y seleccione el directorio raíz de un escenario dividido generado por Atque. El navegador de escenario mostrará en pantalla todos los niveles que tengan un archivo script válido.
 ...
 Cuando comienza a trabajar en un nuevo escenario, o desea editar un escenario que no tiene datos específicos de Hux, debe crear una carpeta dividida (split folder) e importalrla, despues de esto puede [agregar niveles](#editing-scenarios) que requieran datos de terminal.
...
Este archivo es leido y escrito por la aplicación, no es necesaria una edición manual (escepto cuando se convinan cambios, ejemplo via herramientas de diferencias)
...
*NOTA: Hux espera que los archivos del escenario entén en el mismo directorio que la racpeta "Resources" apra el miemo escenario; de lo contrario, no podrá cargar las imágenes a las que se hace referencia en los scripts.*
...
El lado izquierdo de la ventana muestra el [Navegador de Escenarios](#scenario-browser) y una tabla de información para la terminal actualmente selecionada.

El lado derecho muestra un [Navegador de Pantalla](#screen-browser) para el terminal seleccionado, una tabla que contiene información sobre la pantalla selecionada actualmente y una vista previa de la terminal.
...
Cada nivel en el escenadio esta representado por una carpeta en la vista de lista.

Hacer doble click en un nivel, aparecerá una lista de las terminales contenidos en dicho nivel.

Al hacer clic en un termina, se mostrará su contenido en el [Explorador de Pantalla](#screen-browser). Para volver a la vista princippal del escenario, utilice el botón "Up" (Arriba) sobre el *Explorador de Escenarios*
...
Al hacer clic en cualquier elemento de la pantalla, se mostrará su informacion y aprecerá una vista previa en la pantalla.

Lo botones debajo de la pantalla tambien se pueden usar para navegar entre las pantalla del terminal selecionado.
...
The [Explorado de Escenarios](#scenario-browser) te permite editar el contenido del escenario, incluidos los niveles y sus terminales.
...
- Las terminales se puede copiar y pegar dentro y entre los niveles. Para copiar los terminales selecionados, haga click derecho y seleciones "Copy", y luego haga click derecho nuevamente en la ubicación deseada y seleciones "Paste".
...
El nombre del nivel es solo para comodidad del usuario. Su nombre para mostrar en el juego debe configurarse en elos scripts relevantes fuera de Hux.

Al agregar un nuevo nivel o reorganizar el escenario, debe proporcionar la carpeta y los nombres de los script correctos. Esto le persmite a Hux sobreescribir los archivos apropiados al exporta el escenario.

*NOTA: ¡la imprementación actual exxcluye caracteres de carpetas y nombres de archivos que no están permitidos en el sistema de archivos de Windows!*
...
El lado derecho contiene una vista de editor donde se puede editar los conteniedos de la pantalla y los metadatos, junto con un vista previa de la pantalla editada actualemnte.

- Puede dar a los terminales un nombre personalizado que para identificarlos fácilmente. Estos datos son específicops de Hux, no se exportan con los archivos de script del terminal.
...
- La edición de la lista de pantallas es similar a la edición de niveles ene el [Navegador de Escenarios](#scenario-browser):
  - Agregar o elimitar pantalla usando los botones debajo den navegador.
  - Mover pantallas usando arrastrar y soltar.
  - Copiar y pegar pantallas seleccionadas click derehoc y seleccionando la acción apropiada.
- Para editar una pantalla, selecciónela en el [Screen Browser](#screen-browser). Esto actutualizará la vista del editor de pantalla y la vista previa. AL cambiar de pantalla, se guardarán los cambios en la última pantalla seleccionada.
...
La página tambien muestra números de línea para ayudar a realizar un seguimiento de qué tan cerca está el contenido del texto del límite de línea.
...
- Las vistas previas para el salto de página aun no estan implementadas. Se recomienda que no permia que el texto exceda el límite de línea, ya uqe esto permite un control menos preciso sobre el diseño del texto.
...
Al guardar por pirmera vez, se le pedirá un nombre y una ubicación para el escenario. Los guardados subsiguientes sobrescribirán este archivo. Para guardar el escenario en una ubicación diferente, puede usar _File->Save Scenario As_.

*NOTA: ¡los datos específicos del editor (por ejemplo, los nombres de terminales) solo se gudardan en el archivo JSON. ¡Estos datos se pierden si el usuarios intenta volver a cargar un escenarios desde los archivos .txt exportados!*

Tambien se le puede solicitar al usiario que guarde al salir (esto omitirá los pasos anteriores).
...
Para exportar un escenario, haga click en _File -> Export Scenario Scripts_, que abre un cuadro de diálogo de exportación. Esto mostrará una lista de todos los niveles y le permitirá al usuario obtener una vista preva del contenido del script. Al hacer clock en un nivel de la lista, se mostrará el script de terminal Aleph One generado.

Al hacer click en OK, se le pedirá al usuario que seleccione una carpeta de destino. Esta puede ser una carpeta dividida (split folder) existente, en cuyo caso Hux sobreescribirá cualquier script de terminal existente. Si no se encuentra un script coincidente para un nivel, Hux creara una nueva carpeta y un archivo sript.

*NOTA: ¡asegúrese de ajustar los atributos de nivel para que Hux sobrescriba los archivos correctos en las cartpetas correctas!*
